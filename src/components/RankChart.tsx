    ...
    return (
      dataSource?.map((item, index) =>
        <div key={item.name} className='grid grid-cols-[48px_1fr_1fr] grid-rows-2 text-xs items-center gap-y-1.5 gap-x-2 px-4 my-2'>
          <div className='row-start-1 col-start-1 row-end-3 col-end-2 flex justify-center items-center size-12 rounded-3xl text-2xl bg-[#EFEFEF]'>{item.sign}</div>
          <div className='self-end col-start-2 col-end-3 row-start-1 row-end-2'>
            {item.name} - {`${(parseFloat(item.value.toString()) / total * 100).toFixed(0)}%`}
          </div>
          <div className='self-end col-start-3 col-end-4 row-start-1 row-end-2 text-right'>
            {item.value} 元
          </div>
